// screens/print.jsx — printable export, styled after Monarch Center handouts
function PrintScreen({ onNav, tweaks }) {
  const [layout, setLayout] = React.useState('grid'); // grid | checklist | schedule
  const [cols, setCols] = React.useState(3);
  const [showTimes, setShowTimes] = React.useState(true);
  const [showFooter, setShowFooter] = React.useState(true);
  const [paper, setPaper] = React.useState('letter'); // letter | a4
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', minHeight: 0 }}>
      ...
      <header className="topbar" style={{ padding: '12px 22px' }}>
        <button className="btn btn--ghost btn--icon" type="button"
                onClick={() => onNav('builder')} title="Back to builder">
          <IconArrowL />
        </button>
        <div className="stack-tight">
          <div className="h2">Print preview</div>
          <div className="meta" style={{ fontSize: 12 }}>Morning Routine · printable handout</div>
        </div>
        <div className="spacer" />
        <button className="btn btn--soft" type="button"><IconShare />Send to caregiver</button>
        <button className="btn btn--primary" type="button" onClick={() => window.print()}>
          <IconPrint />Print
        </button>
      </header>

      {/* Body: settings rail + paper preview */}
        ...
        <aside style={{
          ...
          <div className="col" style={{ gap: 10 }}>
            ...
            <div className="meta" style={{ fontSize: 12 }}>
              Daybook handouts include a soft footer with a credit and a notes line.
              You can replace the credit with your clinic or school.
            </div>
            <button className="btn btn--soft" type="button"
                    style={{ alignSelf: 'flex-start', height: 30, fontSize: 13 }}>
              <IconPlus style={{ width: 13, height: 13 }} /> Add logo
            </button>
          </div>
        </aside>
      ...
      <style>{`
        @media print {
          ...
          .topbar, .sidebar, aside, .twk-panel { display: none !important; }
...
// ── The paper page ─────────────────────────────────────────────────────
function Page__Paper({ paper, layout, cols, steps, showTimes, showFooter }) {
  const ratio = paper === 'a4' ? 1 / 1.414 : 1 / 1.294; // letter 8.5x11
  const width = 760; // px in preview
  ...
  return (
    <div className="print-paper" style={{
      ...
      <div style={{ marginTop: 22, marginBottom: 18, textAlign: 'center' }}>
        <div style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-.005em' }}>
          Morning Routine
        </div>
        <div style={{
          ...
          <span>Weekdays · starts 7:00 AM</span>
          <span style={{ width: 3, height: 3, borderRadius: '50%', background: '#B1B3A8' }} />
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        {layout === 'grid' && <PaperGrid steps={steps} cols={cols} showTimes={showTimes} />}
        {layout === 'checklist' && <PaperChecklist steps={steps} showTimes={showTimes} />}
        {layout === 'schedule' && <PaperSchedule steps={steps} />}
      </div>

      {showFooter && <PaperFooter />}
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'right', fontSize: 10.5, color: '#8A8E83', lineHeight: 1.4 }}>
        Printed Tue, Mar 18<br />
        daybook.app/b/morning-sam
      </div>
    </div>
  );
}
    ...
    <div style={{ marginTop: 18, paddingTop: 12, borderTop: '1px solid #E5DECE' }}>
      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
        ...
        <div style={{ minWidth: 180, textAlign: 'right' }}>
          ...
          <div style={{ fontSize: 12, color: '#1A1D1A', fontWeight: 700 }}>Maya · David · Grandma</div>
        </div>
      </div>
...
// ── Grid layout (the main one — Monarch "Bedtime Routine" / "Summer Schedule" style) ──
...
// ── Checklist layout (like "Take Out Clothes") ──
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ fontSize: 12, color: '#5A6058', marginBottom: 4, fontStyle: 'italic', textAlign: 'center' }}>
        Put a check next to each step after Sam finishes it…
      </div>
      {steps.map((s, i) => {
        const cat = CATEGORIES[s.category];
        return (
          <div key={s.id} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: '#5A6058', marginTop: 2 }}>{s.note}</div>
            </div>
            {showTimes && (
              <div style={{ textAlign: 'right', minWidth: 76, color: '#1A1D1A' }}>
                <div style={{ fontSize: 15, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{s.time}</div>
                <div style={{ fontSize: 10.5, color: '#8A8E83' }}>{s.duration} min</div>
              </div>
            )}
          </div>
        );
...
// ── Schedule layout (like "School Days Morning Schedule") ──
function PaperSchedule({ steps }) {
  // Group into morning blocks
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      <div style={{
        display: 'grid', gridTemplateColumns: '74px 1fr',
        borderTop: '2px solid #1A1D1A',
      }}>
        {steps.map((s, i) => {
          const cat = CATEGORIES[s.category];
          return (
            <React.Fragment key={s.id}>
              <div style={{
                ...
                {s.time}
                <div style={{ fontSize: 11, color: '#8A8E83', fontWeight: 400, marginTop: 2 }}>{s.duration} min</div>
              </div>
              <div style={{
                display: 'flex', alignItems: 'center', gap: 14,
                padding: '12px 14px',
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11.5, color: '#5A6058' }}>{s.note}</div>
                </div>
                <div style={{ flex: 1 }} />
                <div style={{
                  fontSize: 10, fontWeight: 700, letterSpacing: '.10em', textTransform: 'uppercase',
                  color: cat.ink,
                  background: cat.bg, padding: '3px 8px', borderRadius: 999,
                }}>{cat.label}</div>
              </div>
            </React.Fragment>
          );
